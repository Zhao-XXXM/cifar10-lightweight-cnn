...
def main():
    ...
    print(f"\n🚀 开始强化版 ResNet-18（数据增强 + Cosine Scheduler）10 个 Epoch 训练...")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            best_model_path = os.path.join(save_dir, "resnet18_augmented_best.pth")
            torch.save(model.state_dict(), best_model_path)
            print(f"  🔥 创历史新高！最佳权重已保存至: {best_model_path} (Val Acc: {best_acc:.2f}%)")

    print(f"\n🎉 训练完成！强化版 ResNet-18 最终最高验证集准确率: {best_acc:.2f}%")
