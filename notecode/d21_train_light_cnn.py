...
def main(args):
    ...
    print(f"Day21: LightVGG-Slim 训练 | device={device}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_acc > best_acc:
            ...
            torch.save(
                model.state_dict(),
                checkpoint_dir / "light_vgg_slim_best.pth",
            )

    elapsed = time.perf_counter() - start_time
    history_path = checkpoint_dir / "light_vgg_slim_history.json"
    ...
    print(f"权重保存至: {checkpoint_dir / 'light_vgg_slim_best.pth'}")
